...
Shader::Shader(const std::string& vertexSrc, const std::string& fragmentSrc)
{
	...
	GLCall(glAttachShader(m_Handle, vs));
	GLCall(glAttachShader(m_Handle, fs));
...
Shader::~Shader()
{
	std::cout << "Shader destroyed: " << m_Handle << "\n";
	glDeleteProgram(m_Handle);
}
...
void Shader::UploadUniformFloat2(const std::string& name, const Vec2D& value)
{
	GLint loc = glGetUniformLocation(m_Handle, name.c_str());
	//	if (m_UniformLocations.find(name) != m_UniformLocations.end())
	if (loc != -1)
...
void Shader::UploadUniformFloat3(const std::string& name, const Vec3D& value)
{
	GLint loc = glGetUniformLocation(m_Handle, name.c_str());
	//	if (m_UniformLocations.find(name) != m_UniformLocations.end())
	if (loc != -1)
